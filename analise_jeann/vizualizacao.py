...
def viz_simples(df: pd.DataFrame, escolha_coluna: str, colunas: list, dado_especifico=""):
    ...
    df_tend = fu.agrupa_por(df, escolha_coluna)
    ...
    if dado_especifico == "":
        ...
        for i in range(0, 4):
            for j in range(0, 6):
                ...
                ax[i, j].set_title(f"{colunas[cont]}")
...
def viz_dist_freq(df: pd.DataFrame, escolha_coluna: str, colunas: list, dado_especifico=""):
    ...
    df_freq = fu.agrupa_por(df, escolha_coluna)
...
def viz_correlativa(df: pd.DataFrame, escolha_coluna: str, colunas: list):
    ...
    # tratar a escolha_colunas e colunas...
    ...
    df_corr_agua = df[[escolha_coluna] + colunas[0]].groupby(escolha_coluna).agg(lambda x: sum(x))
    ...
    df_corr_energia = df[[escolha_coluna] + colunas[1]].groupby(escolha_coluna).agg(lambda x: sum(x))
    ...
    df_corr_esgoto = df[[escolha_coluna] + colunas[2]].groupby(escolha_coluna).agg(lambda x: sum(x))
    ...
    df_corr_lixo = df[[escolha_coluna] + colunas[3]].groupby(escolha_coluna).agg(lambda x: sum(x))
...
def viz_clusterizada(df: pd.DataFrame):
    pass # a fazer...